...
import year2015.models.day06.Entrada;
...
public class Day06 {
    ...
    private static String[] entrada = new String[]{"toggle 461,550 through 564,900",
    ...
    public static void main(String[] args) {
        int[][] luzes = iniciaLuzesApagadas();
        var entradaList = entradaAjustada();
        entradaList.forEach(eL -> {
            switch (eL.getAcao()) {
                case TOGGLE:
                    for (int i = eL.getLinhaInicio(); i <= eL.getLinhaFim(); i++) {
                        for (int j = eL.getColunaInicio(); j <= eL.getColunaFim(); j++) {
                            luzes[i][j] += 2;
                        }
                    }
                    break;
    ...
    private static int[][] iniciaLuzesApagadas() {
        int[][] luzes = new int[1000][1000];
        for (int i = 0; i <= 999; i++) {
            for (int j = 0; j <= 999; j++) {
                luzes[i][j] = 0;
            }
    ...
    private static List<Entrada> entradaAjustada() {
        List<Entrada> entradaList = new ArrayList<>();
        Arrays.asList(entrada).forEach(e -> {
            String acao;
            String[] inicio;
            String[] fim;
            if (e.startsWith(TOGGLE)) {
                acao = TOGGLE;
                inicio = e.substring(e.indexOf(TOGGLE) + 6, e.indexOf(THROUGH)).trim().split(",");
            } else if (e.startsWith(TURN_OFF)) {
                acao = TURN_OFF;
                inicio = e.substring(e.indexOf(TURN_OFF) + 8, e.indexOf(THROUGH)).trim().split(",");
            } else {
                acao = TURN_ON;
                inicio = e.substring(e.indexOf(TURN_ON) + 7, e.indexOf(THROUGH)).trim().split(",");
            }
            fim = e.substring(e.indexOf(THROUGH) + 7).trim().split(",");
            entradaList.add(new Entrada(acao, Integer.parseInt(inicio[0]), Integer.parseInt(inicio[1]),
                    Integer.parseInt(fim[0]), Integer.parseInt(fim[1])));
        });

        return entradaList;
    }

    private static void resposta(int[][] luzes) {
        int contadorBrilho = 0;
        for (int i = 0; i <= 999; i++) {
            for (int j = 0; j <= 999; j++) {
                if (luzes[i][j] > 0) {
                    contadorBrilho += luzes[i][j];
                }
            }
        }
        System.out.println("Quantidade de brilho: " + contadorBrilho);
    }
}
